...
function setup() {
  canvas = createCanvas(800, 800).parent("sketch-wrapper");
  
  background(220);
  pointsLayer = createGraphics(800,800);
  sliderDiv = createDiv();
  sliderDiv.position(canvas.position().x,canvas.position().y+height-130)
           .size(130,130)
...
function drawSliderValues(){
  for(let slider of sliders){
    console.log(sliderDiv.position().x + slider.position().x + sliderDiv.width);
